...
image() {
  ...
  echo "Building and pushing cyphernode/knots for $arch tagging as ${version} alpine arch ${arch2}..."

  docker build --no-cache -t cyphernode/knots:${arch}-${version} --build-arg ARCH=${arch2} . \
  && docker push cyphernode/knots:${arch}-${version}

  return $?
}

manifest() {
  echo "Creating and pushing manifest for cyphernode/knots for version ${version}..."

  docker manifest create cyphernode/knots:${version} \
                         cyphernode/knots:${x86_docker}-${version} \
                         cyphernode/knots:${arm_docker}-${version} \
                         cyphernode/knots:${aarch64_docker}-${version} \
  && docker manifest annotate cyphernode/knots:${version} cyphernode/knots:${arm_docker}-${version} --os linux --arch ${arm_docker} \
  && docker manifest annotate cyphernode/knots:${version} cyphernode/knots:${x86_docker}-${version} --os linux --arch ${x86_docker} \
  && docker manifest annotate cyphernode/knots:${version} cyphernode/knots:${aarch64_docker}-${version} --os linux --arch ${aarch64_docker} \
  && docker manifest push -p cyphernode/knots:${version}
